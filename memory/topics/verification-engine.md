...
### 29. 组织政治学领域测试 + AI 自引验证 (2026-06-11)

Skill 首次用于组织政治学领域——体制内权力圈层形成机制验证。4 轮问答覆盖文本内验证→逻辑漏洞检测→联网实证→AI 自引核验。

**领域特征**：证据金字塔从健康类 Meta/RCT 变为政治学 DID/历史制度分析。claimType 以 causal+normative 为主（vs 健康类 factual 为主）。来源类型 personal_note 封顶 LOW，但外部文献引用可升至 MEDIUM。

**关键外部文献**：
- Walder 1986 新传统主义庇护关系理论 / Oi 1989 / 周雪光 2017 制度逻辑
- Jiang AJPS 2018 庇护网络绩效激励 / Su & Tao Cambridge 2024 贤能 vs 庇护
- SciOpen 2025 官员晋升联动 DID（N=7910）/ Xiao et al. 2025 中央表彰快速通道

**AI 自引验证发现**：娜娜上轮引用的研究结论经原文核验——2 条 HIGH（精确匹配）、1 条 MEDIUM（综合判断量级证据不足）、1 条 LOW（概念框架不可验证）。发现：①漏了第四条晋升路径（中央表彰）②"绩效评估无约束力"省略了 nuance（绩效在有 patron 时有效）。

**领域适配性**：组织政治学在 Skill 内适配度 ⭐⭐½——与历史/程序类类似，缺政治学专属证据类型（DID/田野调查/过程追踪），但文献引用+制度共识路径可用。

### 31. 记忆系统自我验证 + 文革编年史验证 (2026-06-12)

**记忆维护自我验证**：汤姆要求用 claim-verification 验证娜娜自己对"记忆维护"的解释。5 条主张全走 pipeline，发现：数字美化了（155→45 实际 185→49，偏差 19%），断言绝对化了（"不再打架"→实际只是加了冲突解决规则）。Lesson: plausible-sounding error 自己也会犯——不是故意，是回忆偏差。在自己的工具上自我验证是避免这类错误的最低成本手段。

**历史领域极限测试**：文革编年史验证是 skill 至今最大规模的单一文本验证——Song Yongyi (Sciences Po) 完整编年史，四种阶段，五种屠杀类型，跨越 1966-1977。11 条主张逐条交叉核验 Sciences Po 原文 + 5 外部独立源（JSTOR/RFI/Wikipedia/郑义/公安部报告），11/11 精确匹配。操作经验：超长学术网页（60KB+）tavily_extract 后 Read 本地文件更可靠。

**领域覆盖扩展**：健康 → 消费 → 组织政治 → 投资 → 历史编年史。已验证领域适配度：
- 健康/医学: ⭐⭐⭐⭐⭐（Meta/RCT 完美映射）
- 产品/消费: ⭐⭐⭐（缺第三方检测数据，瓶颈在输入）
- 组织政治: ⭐⭐½（文献引用可用，缺 DID/田野调查专属证据类型）
- 投资/金融: ⭐⭐⭐（DALBAR 等机构报告可作 evidence）
- 历史编年史: ⭐⭐⭐⭐（多独立学术源可交叉验证，但需外部搜索）

## 变更记录
- 2026-06-12: 记忆系统自我验证（5 条主张交叉核验 git）+ 文革编年史验证（11/11 精确匹配）。领域覆盖扩展至历史编年史。
- 2026-06-11: claim-verification Skill 模式重构 — multi-source(默认)/text-internal 双模式 + external_verification 证据类型 + 02-SOUL Skill 边界例外。04-MEMORY 155→45行导航化。投资抄底验证(多源模式首次实战)。组织政治学领域测试 + AI 自引验证。发现第四种晋升路径（中央表彰）+ Jiang 研究 nuance 修正。
