...
## DeepSeek 2026年中期全面评估 — 2026-07-12

- **问题类型**：综合研判（6维：融资+竞争+商业化+V4+认知空白+风险）
- **模式**：Deep（1轮3引擎并行搜索，1轮Challenger验证）
- **轮次**：1轮收敛（公开数据极其丰富，510亿融资+芯片自研+V4正式版三期事件叠加）
- **最佳查询**：
  - `DeepSeek 510亿融资 首轮 投资方 估值`（You.com，中文）→ 直接命中《财经》深度+每经+工商信息
  - `DeepSeek closes record funding unusual deal structure`（Exa，英文）→ 命中The Information首发
  - `DeepSeek V4 preview release 2026 benchmarks`（Exa，英文）→ 命中Hugging Face技术博客+arXiv论文
  - `DeepSeek 大规模招聘 商业化 商业模式`（You.com，中文）→ 命中财新+凤凰科技+界面
  - `DeepSeek AI chip development Nvidia export controls July 2026`（Exa，英文）→ 命中Reuters独家+Ars Technica分析
- **最佳源**：
  - 《财经》杂志——融资过程最深度调查（投前估值3500亿+5%ESOP的精确数据、FA乱象、~100家穿透参与者）
  - The Information——融资架构首发（LP结构+锁定期+投票权+5年锁定期）
  - 路透社——自研芯片独家（3位知情人士，inference-only芯片，一年前启动）
  - Hugging Face博客+arXiv论文——V4最完整基准数据+技术细节（CSA/HCA/mHC/Muon优化器）
  - 每经——国资投资人分析+产业资本主导时代判断+估值从$10B→$55B的时间线
  - 财新——招聘规模翻倍+AGI宣言
- **Challenger 发现的修正**：4/10
  1. **SIGNIFICANT**: F1估值使用报道区间上沿($55B)，财经数据为投前~3675亿含ESOP。多数外媒用">$50B"。修正为区间表述
  2. **SIGNIFICANT**: F7的96.8%缺"基础大模型细分赛道"限定词，不加会严重误导（Q1全AI风投2560亿，三家远不到96.8%）
  3. **MODERATE**: F9人才流失叙事缺失整体基线——391名作者中仅25人离职(6.4%)，top15作者仅2人离职。"5人离职"清单准确但叙事权重被放大
  4. **MODERATE**: F10定性"接近国际前沿"低估实际竞争力——LiveCodeBench 93.5(全部最高)、SWE Verified持平Opus 4.6、MCPAtlas几乎并列。应为"coding/agent进入第一梯队"
- **死胡同**：Tavily Search API超限（432 usage limit）
- **最脆弱发现**（已修正）：
  - "投后估值4000亿/ $55B"——是报道区间的上沿而非共识值。修正为$50B-$55B
  - "三家拿走96.8%融资"——缺失关键限定词。修正为"基础大模型细分赛道96.8%"
  - "人才大量流失"——缺失整体留存率基线。补充93.6%留存率作为反向语境
- **教训**：
  - **中文财经媒体的深度调查质量不亚于国际大报**：《财经》的融资过程调查（投前估值/ESOP细节/FA乱象/~100家穿透）和每经的国资投资人分析，在深度和颗粒度上不逊于甚至超过The Information和Reuters。搜索策略应给中文深度财经源平等权重
  - **DeepSeek参与方远比表面10家多**：穿透基金结构后发现~100家机构/个人。IDG的保险资金特征、Monolith的浙沪国资LP、溥泉资本的地方政府背景——这些"谁在出钱"的信息比"出了多少钱"更有分析价值
  - **"不透明"是双刃剑**：DeepSeek收入/利润从未公开，所有营收数据(API $605M/总$1.1B)均为第三方估算。没有任何可审计的财务数据支撑——这是所有关于DeepSeek分析的最大结构性问题
  - **叙事密度在6-7月出现峰值**：510亿融资(6/16)+全员翻倍招聘(6/25)+峰谷定价官宣(6/29)+V4正式版(待7月中)+芯片自研曝光(7/7)+中国AI出口限制(7/10)——六个重大信号在不到一个月内密集释放，叙事突变已确认
  - **Challenger再次证明结构价值**：4条重要修正中有2条会显著改变读者对DeepSeek的判断（估值从"确定$55B"变为"区间$50B-$55B"；96.8%从"融资垄断"变为"细分赛道集中"）——构建者自己不会发现这些盲区
