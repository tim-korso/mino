...
- **bash pipe 打断 env var (2026-07-21)**: `VAR=val cmd1 | cmd2` 只把 env var 传给管道中的第一个命令 (`cmd1`)，不传给 `cmd2`。在 bash 管道中需要把 env var 传递给下游时，必须用 `export VAR=val` 或者 `VAR=val; cmd1 | cmd2`。这是 bash 变量作用域和 pipeline subshell 机制的交互——每个管道段是一个独立 subshell，变量声明只对当前段可见。实测：`MD_HUMAN=true echo "data" | python3 -c "import os; print(os.environ.get('MD_HUMAN'))"` → `None`；`export MD_HUMAN=true; echo "data" | python3 -c "..."` → `'true'`。
- **macOS 26 systemsetup 全命令需 admin (2026-07-21)**: 24 个 get/set 子命令全部存在于二进制中（通过内嵌 plist 枚举验证），但每个 `-get*` 命令都要求管理员权限。这是 macOS 26 新增的安全限制——旧版中部分 get 命令（如 `-getcomputername`）不需要 admin。绕过：`scutil --get ComputerName` (无需 sudo)，`readlink /etc/localtime` (时区)，`pmset -g` (电源)，`networksetup` (网络)。
- **macOS BSD grep 的 "Binary file" 陷阱 (2026-07-21)**: macOS BSD grep (非 GNU grep) 在通过管道 (`|`) 处理来自 PlistBuddy 的输出时，可能检测到二进制内容并输出 "Binary file (standard input) matches" 到**stdout**，污染管道下游的 JSON 解析。用 `grep -aE` 替代 `grep -E`——`-a` flag 强制按文本处理（等同于 `--text`），消除误判。GNU grep 不会有此行为。
- **haiku 模型别名已漂移 (2026-07-21 moonsohot provider 实测)**: `haiku` 在 moonshot provider 下指向的模型已下架——API 调用立即报错。当前可用别名: `fable`/`sonnet`/`opus` → kimi-k2.6; 省略继承会话 (kimi-k3)。结论：provider 的 `modelAliases` 配置会随时间漂移——不要硬编码别名字符串，用探针 (`agent(prompt, {model: 'xxx'})` 空跑一次) 在使用前验证。
- **PlistBuddy 比 defaults write 多了一层 (2026-07-21)**: `defaults write` 只能操作顶层 key。`/usr/libexec/PlistBuddy -c "Set :nested:key:path value"` 可以按 key-path 编辑任意深度的值——包括 `FlClash flutter.config` 这种嵌套 JSON blob（在 plist 里存为 Dict）。这对自动化配置管理是关键能力——`defaults` 对大 JSON blob 容易截断或失败。
- **corebrightnessdiag 可用 (2026-07-21)**: `/usr/libexec/corebrightnessdiag` 有 5 个未文档化子命令 (`nightshift`, `sunschedule`, `nightshift-internal`, `status-info`, `status-info-internal`)，全部无需 sudo。Night Shift 的完整状态机（包括太阳调度、内部校准状态）可以通过此工具读取——GUI 只有 on/off toggle。
