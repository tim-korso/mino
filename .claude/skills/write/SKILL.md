...
当用户说 "write continue" 时，**调用 `write-continue` Workflow 自动检测书的状态并执行所有可自动化步骤**。

### 不再手动跑检查清单——Workflow 自动做

```javascript
export const meta = {
  name: 'write-continue',
  description: '自动检测书的状态→Phase1基础建设+Phase2深化缺口报告',
  phases: [
    { title: '检测状态', detail: '扫描文件+DB判断缺什么' },
    { title: 'Phase1 基础建设', detail: '并行执行提取/验证/扫描' },
    { title: 'Phase2 深化检测', detail: '扫描标记→报告缺口→自动填充适配模板' },
  ],
}
...
const STATE_SCHEMA = {
  ...
  properties: {
    book_id: { type: 'string' },
    chapters_written: { type: 'array', items: { type: 'string' }, description: '已写章节的文件名列表' },
    chapter_count: { type: 'number' },
    chapters_missing: { type: 'array', items: { type: 'string' }, description: '骨架中有但未写的章节' },
    classics_registered: { type: 'number', description: 'db中已注册的该领域经典数' },
    classics_extracted: { type: 'number', description: '已完成4-pass深层提取的经典数' },
    extraction_needed: { type: 'boolean' },
    deep_notes_status: { type: 'string', description: '哪些章有/缺经典深层注' },
    phase2_gaps: {
      type: 'array',
      items: { type: 'string', enum: ['A_history', 'B_comparison', 'C_signals', 'D_controversy', 'E_reading', 'F_myths', 'G_discovery', 'H_self_assess', 'I_opposition'] },
      description: '缺失的Phase2深化类型'
    },
    actions_available: { type: 'array', items: { type: 'string' }, description: '可执行的Phase1动作列表' },
  },
  required: ['chapters_written', 'extraction_needed', 'phase2_gaps', 'actions_available']
}

const state = await agent(
  `你是书状态检测Agent。扫描以下路径判断书的完成状态。

工作区: workspace/${book_id}/

## 检测项目

### 章节状态
- 用Glob扫描 workspace/${book_id}/*.md
- 排除 00-骨架.md 和 08-*.md（附录）
- 列出已写章节（如 01-xxx.md）
- 对比骨架中的骨头数，列出缺失的章节

### 经典提取状态
- 检查 db.py stats 或查询 classics 表
- 该领域共注册多少经典？有多少已完成4-pass提取？
- 如果有未提取的→ extraction_needed: true
...
### Phase 2 深化缺口
扫描各章和附录，检测以下标记/内容是否存在：
- A_history: 各章是否含 ★历史深化 标记？
- B_comparison: 是否有≥3个跨领域比较案例？
- C_signals: 是否有附录列出关键指标+频率+警戒值？
- D_controversy: 是否有争议图鉴附录（★诚实标记）？
- E_reading: 是否有深度阅读路径附录？
- F_myths: 各章是否含 ★误区爆破 标记（每章≥2个）？
- G_discovery: 各章是否含 ★发现故事 标记（每章1个）？
- H_self_assess: 是否有读者自评工具附录？
- I_opposition: 是否有反对声音附录（★反对声音）？

### 可执行动作
列出所有可立即执行的Phase 1动作：
- extraction_needed → "extract_classics"
- chapters_missing → "write_chapters"
- deep_notes_gaps → "add_deep_notes"
- 等等
...
log(`${state.chapter_count} 章已写${state.chapters_missing.length > 0 ? '，缺' + state.chapters_missing.length + '章' : '，齐全'} | 经典提取: ${state.classics_extracted}/${state.classics_registered} | Phase2缺口: ${state.phase2_gaps.length}项`)

// ═══ Phase 1: 并行执行所有可自动化任务 ═══
phase('Phase1 基础建设')
...
if (state.extraction_needed) {
  phase1Actions.push(
    (async () => {
      log('启动经典提取 Workflow...')
      // 调用经典提取 pipeline
      const books = await agent(`列出 ${domain} 领域所有未提取的经典（从 claims.db 查询）`, { effort: 'low' })
      return { type: 'extraction', status: 'queued', count: books }
    })()
  )
}

const phase1Results = (await Promise.all(phase1Actions)).filter(Boolean)
log(`Phase1: ${phase1Results.length} 个任务已启动`)

// ═══ Phase 2: 深化检测 + 自动填充 ═══
phase('Phase2 深化检测')

// 只报告缺口——不自动填充（模板A-I需要话题适配，交给AI在主会话中执行）
const gapReport = state.phase2_gaps.length > 0
  ? state.phase2_gaps.map(g => {
      ...
      return `- **${labels[g]}**（模板${g.split('_')[0].toUpperCase()}）`
    }).join('\n')
  : '✅ 全部Phase2深化已完成'
...
return {
  state: {
    chapters: `${state.chapter_count}章已写`,
    extraction: `${state.classics_extracted}/${state.classics_registered}已提取`,
    deep_notes: state.deep_notes_status,
  },
  phase1_dispatched: phase1Results,
  phase2_gaps: gapReport,
  phase2_gap_list: state.phase2_gaps,
  next_actions: state.actions_available,
}
```
...
```bash
# 在 /write continue 时，调用 Workflow 自动检测：
Workflow({
  name: 'write-continue',
  args: { book_id: 'exchange-value', domain: 'exchange-value' }
})
# 输出 → 状态报告 + Phase1自动执行 + Phase2缺口清单
```

### 手动 vs 自动

| | 旧版（手动检查清单） | 新版（write-continue Workflow） |
|---|---|---|
| 状态检测 | AI手动读文件、查DB、判断 | 一个Agent自动扫描→结构化输出 |
| Phase1执行 | AI手动跑每一步 | 所有可并行的Phase1任务自动启动 |
| Phase2缺口 | AI手动grep各章 | 一个Agent自动扫描9类标记→报告 |
| 时间 | ~3-5min（AI思考+执行） | **~1-2min**（一个Workflow调用） |
| 可靠性 | 取决于AI记不记得所有检查项 | 结构化schema强制覆盖所有检查项 |
